...
-- =============================================================================
-- Preview markdown in QuickMD
-- =============================================================================
-- <leader>v on a markdown buffer opens the file you're on in QuickMD
-- (see config/commands.lua for the :Md command itself).
autocmd("FileType", {
	group = augroup("markdown_preview"),
	pattern = "markdown",
	callback = function(event)
		vim.keymap.set("n", "<leader>mp", "<cmd>Md<CR>", {
			buffer = event.buf,
			silent = true,
			desc = "Preview file in QuickMD",
		})
	end,
})
